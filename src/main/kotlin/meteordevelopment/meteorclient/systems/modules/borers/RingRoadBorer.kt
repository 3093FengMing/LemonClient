package meteordevelopment.meteorclient.systems.modules.borers
...
class RingRoadBorer: LemonModule(Categories.Borers, "RingRoadBorer", "Automatically digs ring road highways.") {
    ...
    fun tick(event:TickEvent.Pre) {
        ...
        if (this.playerPos != this.prevBlockPos || Util.getMeasuringTimeMs() - this.lastUpdateTime > 800) {
            ...
            when (this.mode) {
                ...
                Mode.HIGHWAY -> {
                    this.doHighway3(playerPos.add(xOffset, 0, zOffset))
                    if (jumping) {
                        this.doHighway3(playerPos.add(xOffset * -1, 0, zOffset * -1))
                        this.doHighway3(playerPos.add(xOffset * -3, 0, zOffset * -3))
                        this.doHighway3(playerPos.add(xOffset * -7, 0, zOffset * -7))
                    } else {
                        this.doHighway3(playerPos.add(xOffset * -3, 0, zOffset * -3))
                    }
                }
    ...
    private fun doHighway3(plyerPos:BlockPos = playerPos) {
