...
import meteordevelopment.meteorclient.utils.world.CityUtils;
import meteordevelopment.meteorclient.utils.player.Interaction;
import meteordevelopment.meteorclient.utils.network.PacketUtils2;
import meteordevelopment.meteorclient.utils.render.NewRenderUtils;
import meteordevelopment.meteorclient.utils.render.RenderInfo;
import meteordevelopment.meteorclient.utils.other.Task;
...
import static meteordevelopment.meteorclient.utils.render.NewRenderUtils.render;
import static meteordevelopment.meteorclient.utils.world.BlockInfo.getState;
import static meteordevelopment.meteorclient.utils.world.BlockInfo.isAir;
...
public class AutoCity extends Module {
    ...
    private final Setting<Boolean> autoswitch = sgGeneral.add(new BoolSetting.Builder().name("auto-switch").description("Mined blocks will be removed faster.May cause desync if server is strict.").defaultValue(true).build());
    ...
    private final PacketUtils2 silentMine = new PacketUtils2();

    public AutoCity() {
        super(Categories.Combat, "auto-city", "Auto City Breaker based on silent mining.");
    }
    ...
    public void onTick(TickEvent.Post event) {
        ...
        if (autoswitch.get() && silentMine.isReady()) mc.world.setBlockState(blockPos, Blocks.AIR.getDefaultState());
    ...
    public void onRender(Render3DEvent event) {
        RenderInfo ri = new RenderInfo(event, NewRenderUtils.RenderMode.Shape, shapeMode.get());
