...
public class CityBreaker extends Module {
    ...
    private void onTick(final TickEvent.Pre event) {
        ...
        if (TargetUtils.isBadTarget(target, targetRange.get())) {
            ...
            blockPosTarget = null;
            breakTimer = 0;
            Notifications.send("No target founded! Disabling...", notifications);
            toggle();
            return;
        }
        ...
        if (delay() == 0) {
            Notifications.send("No pick in hotbar! Disabling...", notifications);
            toggle();
            return;
        }

        if (blockPosTarget == null) {
            Notifications.send("No city block found! Disabling...", notifications);
            toggle();
            return;
        }

         if (PlayerUtils.distanceTo(blockPosTarget) > mc.interactionManager.getReachDistance()) {
             Notifications.send("Block to mine too far! Disabling...", notifications);
             toggle();
             return;
        }
        ...
        if (!antiSpam) {
            Notifications.send("breaking " + target.getGameProfile().getName() +" surround", notifications);
            if (support.get()) {
