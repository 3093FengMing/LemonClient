...
import meteordevelopment.meteorclient.managers.Managers;
import meteordevelopment.meteorclient.mixin.SoundMixin;
...
import meteordevelopment.meteorclient.systems.modules.player.AutoMine;
...
import meteordevelopment.meteorclient.utils.misc.IntTimerList;
import meteordevelopment.meteorclient.utils.other.SettingUtils;
import meteordevelopment.meteorclient.utils.player.InvUtils;
import meteordevelopment.meteorclient.utils.player.LDamageUtils;
...
public class LemonAura extends Module {
    public LemonAura() {super(Categories.Combat, "lemon-aura", "automatically Breaks and places crystals (but better).");}
    ...
    private final Setting<FadeMode> fadeMode = sgRender.add(new EnumSetting.Builder<FadeMode>()
        ...
        .visible(() -> renderMode.get() == RenderMode.Lemon)
        .build()
    );
    ...
    private final Setting<Double> animationSpeed = sgRender.add(new DoubleSetting.Builder()
        ...
        .min(0)
        .sliderRange(0, 10)
        .visible(() -> renderMode.get().equals(RenderMode.Lemon))
        .build()
    );
    ...
    private final Setting<SettingColor> lineColor = sgRender.add(new ColorSetting.Builder()
        ...
        .defaultValue(new SettingColor(255,255,255,105))
        .build()
    );
    ...
    private void onTickPre(TickEvent.Pre event) {
        ...
        if (mc.player != null && mc.world != null) {
            ...
            extPos = getExtPos();
            if (debugRange.get()) {
                info(debugRangeHeight(1) + "  " + dist(debugRangePos(1)) + "\n" +
                    ...
                    SettingUtils.attackRangeTo(LemonKUtils.getCrystalBox(new BlockPos(debugRangeX.get(), debugRangeY.get(), debugRangeZ.get())), new Vec3d(debugRangeX.get() + 0.5, debugRangeY.get(), debugRangeZ.get() + 0.5)));
            }
        }
    ...
    private void onSound(PlaySoundEvent event) {
        if (explodeSound.get()) {
            if (event.sound.getId() == SoundEvents.ENTITY_GENERIC_EXPLODE.getId()) {
                ((SoundMixin) event.sound).setID(SoundEvents.BLOCK_ANVIL_FALL.getId());
                ((SoundMixin) event.sound).setVolume(1);
                ((SoundMixin) event.sound).setPitch(10);
    ...
    void placeCrystal(BlockPos pos, Direction dir, Hand handToUse, int sl, int hsl) {
        if (pos != null && mc.player != null) {
            ...
            if (SettingUtils.shouldRotate(RotationType.Crystal)) {
                Managers.ROTATION.end(LemonKUtils.getBox(pos));
            }
    ...
    double[][] getDmg(Vec3d vec) {
        self = LDamageUtils.crystalDamage(mc.player, extPos.containsKey(mc.player) ? extPos.get(mc.player) : mc.player.getBoundingBox(), vec, null, ignoreTerrain.get());
        ...
        for (Map.Entry<PlayerEntity, Box> entry : extPos.entrySet()) {
            ...
            double dmg = LDamageUtils.crystalDamage(player, box, vec, null, ignoreTerrain.get());
