...
public class AnchorAura extends Module {
    ...
    private final Setting<Boolean> breakSelfTrap = sgAutomation.add(new BoolSetting.Builder()
        .name("break-selftrap")
        .description("Break target's selftrap before placing/breaking.")
        .defaultValue(true)
        .build()
    );
    ...
    private void onTick(TickEvent.Post event) {
        ...
        if (antiStuck.get() && !sentAntiStuck) {
            if (findBreakPos(target.getBlockPos()) == null && findPlacePos(target.getBlockPos()) == null && BlockUtils.getBlock(target.getBlockPos().up(2)) == Blocks.GLOWSTONE) {
                FindItemResult pick = ItemUtils.findPick();
                if (pick.found()) {
        ...
        if (breakSelfTrap.get() && !sentTrapMine) {
            if (findBreakPos(target.getBlockPos()) == null && findPlacePos(target.getBlockPos()) == null && AutomationUtils.isTrapBlock(target.getBlockPos().up(2))) {
                ...
                if (pick.found()) {
                    InvUtils.updateSlot(pick.slot());
                    Notifications.send("Breaking " + target.getEntityName() + "'s self-trap.", notifications);
                    AutomationUtils.doPacketMine(target.getBlockPos().up(2));
                    sentTrapMine = true;
                    return;
                }
        ...
        if (breakBurrow.get() && !sentBurrowMine && AutomationUtils.isBurrowed(target, true)) {
            ...
            if (pick.found()) {
                InvUtils.updateSlot(pick.slot());
                Notifications.send("Breaking " + target.getEntityName() + "'s burrow", notifications);
        ...
        if (placeDelayLeft >= placeDelay.get() && place.get()) {
            ...
            if (placePos != null) {
                placeDelayLeft = 0;
                BlockUtils.place(placePos, anchor, (rotationMode.get() == AnchorAura.RotationMode.Place || rotationMode.get() == AnchorAura.RotationMode.Both), 50);
            }
        }
