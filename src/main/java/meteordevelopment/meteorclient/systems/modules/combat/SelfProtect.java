...
public class SelfProtect extends Module {
    ...
    private final SettingGroup sgAutoAnvil = settings.createGroup("Auto Anvil");
    ...
    private final Setting<Boolean> antiAnvil = sgAutoAnvil.add(new BoolSetting.Builder()
        .name("enabled")
    ...
    public SelfProtect() {
        super(Categories.Combat, "self-protect", "Protects you from various combat modules.");
    }

    @EventHandler
    private void onTick(TickEvent.Pre event) {
        if (antiAnvil.get()) for (int i = 0; i <= mc.interactionManager.getReachDistance(); i++) {
    ...
    private void place(BlockPos blockPos) {
        if (useStringWeb.get() == Use.String && mc.world.getBlockState(blockPos).getBlock().asItem() != Items.STRING) BlockUtils.place(blockPos, InvUtils.findInHotbar(Items.STRING), false, 50, swing.get());
        else if (useStringWeb.get() == Use.Web && mc.world.getBlockState(blockPos).getBlock().asItem() != Items.COBWEB) BlockUtils.place(blockPos, InvUtils.findInHotbar(Items.COBWEB), false, 50, swing.get());
    }
