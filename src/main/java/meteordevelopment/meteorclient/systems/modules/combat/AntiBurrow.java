...
import meteordevelopment.meteorclient.utils.player.BPlayerUtils;
...
public class AntiBurrow extends Module {
    ...
    private final Setting<Boolean> notifications = sgGeneral.add(new BoolSetting.Builder()
            .name("notifications")
            .description("Sends a notifications when it is trying to burrow mine someone.")
            .defaultValue(true)
            .build()
    );
    ...
    public AntiBurrow() {
        super(Categories.Combat, "anti-burrow", "Automatically mines enemy's burrow block.");
    }
    ...
    private void onTick(TickEvent.Pre event) {
        ...
        if (blockPosTarget == null) {
            if (notifications.get()) ToastNotifications.addToast("Burrow target is null");
            toggle();
            mining = false;
            target = null;
            return;
        ...
        if (BPlayerUtils.distanceFromEye(blockPosTarget) > mineRange.get()) {
            if (notifications.get()) ToastNotifications.addToast("Burrow Position is invalid");
            toggle();
            return;
        }
        ...
        if (!pickaxe.isHotbar()) {
            if (notifications.get()) ToastNotifications.addToast("Burrow No pickaxe found");
            toggle();
            return;
        }
        ...
        if (!sentMessage && notifications.get()) {
            ToastNotifications.addToast("Attempting to burrow mine " + target.getEntityName());
            toggle();
            return;
        }
        ...
        if (retry.get()) {
            ...
            if (blockPosTarget != null && ticksPassed <= 0) {
                if (notifications.get()) ToastNotifications.addToast("Retrying...");
                toggle();
            }
        }
