...
import meteordevelopment.meteorclient.mixin.MixinRaycastContext;
...
public class RaytraceSettings extends Module {
    public RaytraceSettings() {
        super(Categories.Settings, "Raytrace", "Global raytrace settings for every blackout module");
    }
    ...
    public boolean placeTrace(BlockPos pos) {
        ...
        switch (placeMode.get()) {
            case SinglePoint -> {
                ...
                result = LDamageUtils.raycast(raycastContext);
    ...
    public boolean attackTrace(Box box) {
        ...
        switch (attackMode.get()) {
            case SinglePoint -> {
                ((IRaycastContext) LDamageUtils.raycastContext).set(mc.player.getEyePos(), new Vec3d((box.minX + box.maxX) / 2f, box.minY + attackHeight.get(), (box.minZ + box.maxZ) / 2f), RaycastContext.ShapeType.COLLIDER, RaycastContext.FluidHandling.NONE, mc.player);

                return LDamageUtils.raycast(LDamageUtils.raycastContext).getType() != HitResult.Type.BLOCK;
            }
            case DoublePoint -> {
                ((IRaycastContext) LDamageUtils.raycastContext).set(mc.player.getEyePos(), new Vec3d((box.minX + box.maxX) / 2f, box.minY + attackHeight1.get(), (box.minZ + box.maxZ) / 2f), RaycastContext.ShapeType.COLLIDER, RaycastContext.FluidHandling.NONE, mc.player);
                if (LDamageUtils.raycast(LDamageUtils.raycastContext).getType() != HitResult.Type.BLOCK) {
                    return true;
                }

                ((IRaycastContext) LDamageUtils.raycastContext).set(mc.player.getEyePos(), new Vec3d((box.minX + box.maxX) / 2f, box.minY + attackHeight2.get(), (box.minZ + box.maxZ) / 2f), RaycastContext.ShapeType.COLLIDER, RaycastContext.FluidHandling.NONE, mc.player);
                return LDamageUtils.raycast(LDamageUtils.raycastContext).getType() != HitResult.Type.BLOCK;
            }
            case Exposure -> {
