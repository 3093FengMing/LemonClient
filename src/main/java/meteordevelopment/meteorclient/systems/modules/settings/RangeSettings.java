...
public class RangeSettings extends Module {
    public RangeSettings() {
        super(Categories.Settings, "Range", "Global range settings for every blackout module");
    }
    ...
    public double placeRangeTo(BlockPos pos, Vec3d from) {
        ...
        switch (placeRangeMode.get()) {
            ...
            case Vanilla -> {
                return getRange(from, LemonKUtils.getClosest(mc.player.getEyePos(), feet, 1, 1));
            }
            case CustomBox -> {
                return getRange(from, LemonKUtils.getClosest(mc.player.getEyePos(), feet, blockWidth.get(), blockHeight.get()));
            }
        }
        return -1;
    }
    ...
    public double attackRangeTo(Box bb, Vec3d feet, Vec3d from) {
        ...
        switch (attackRangeMode.get()) {
            ...
            case Vanilla -> {
                return getRange(from, LemonKUtils.getClosest(mc.player.getEyePos(), feet, Math.abs(bb.minX - bb.maxX), Math.abs(bb.minY - bb.maxY)));
            }
            case Middle -> {
                return getRange(from, new Vec3d((bb.minX + bb.maxX) / 2, (bb.minY + bb.maxY) / 2, (bb.minZ + bb.maxZ) / 2));
            }
            case CustomBox -> {
                return getRange(from, LemonKUtils.getClosest(mc.player.getEyePos(), feet, Math.abs(bb.minX - bb.maxX) * closestAttackWidth.get(), Math.abs(bb.minY - bb.maxY) * closestAttackHeight.get()));
            }
            case UpdatedNCP -> {
