...
import static meteordevelopment.meteorclient.systems.hud.modules.ToastNotifications.addToast;
import static meteordevelopment.meteorclient.utils.world.BlockInfo.*;
import static meteordevelopment.meteorclient.utils.entity.EntityInfo.getBlockPos;
...
public class Notifications extends Module {
    ...
    public Notifications() {
        super(Categories.Render, "Notifications", "Sends messages in hud about different events.");
    }
    ...
    private void onReceivePacket(PacketEvent.Receive event) {
        ...
        synchronized (totemPopMap) {
            ...
            if (totemNotif.get()) send(entity.getEntityName() + " popped " + pops + " totems!", notifications);
        }
    }
        ...
        synchronized (totemPopMap) {
            for (PlayerEntity player : mc.world.getPlayers()) {
                ...
                if (player.deathTime > 0 || player.getHealth() <= 0) {
                    ...
                    if (deaths.get()) send(player.getEntityName() + " died after popping " + pops +" totems!", notifications);
