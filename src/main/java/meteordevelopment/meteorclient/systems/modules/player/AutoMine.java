...
import meteordevelopment.meteorclient.systems.modules.settings.SwingSettings;
...
public class AutoMine extends Module {
    public AutoMine() {super(Categories.Player, "Auto-Mine", "Break block use packet");}
    ...
    private final Setting<SwitchMode> switchMode = sgGeneral.add(new EnumSetting.Builder<SwitchMode>()
        .name("Switch Mode")
        .description(".")
        .defaultValue(SwitchMode.GhostHandBypass)
        .build()
    );
    ...
    private void onTick(TickEvent.Pre event) {
        if (targetPos != null && getBlock(targetPos) != Blocks.AIR) {
            ...
            if (SettingUtils.shouldRotate(RotationType.Breaking)) {
                Managers.ROTATION.start(LemonKUtils.getBox(targetPos), 9, RotationType.Breaking);
            }
        }
    }
    @EventHandler(priority = EventPriority.HIGHEST)
    ...
    private void onRender(Render3DEvent event) {
        ...
        if (waitingToStart != null) {
            ...
            //Render
            Vec3d v = LemonKUtils.getMiddle(targetPos);
                    ...
                    if (at != null) {
                        ...
                        if (rotated) {
                            ...
                            if (holdingBest(targetPos)) {
                                holding = 1;
                            } else if (InvUtils.invSwitch(fastestSlot(targetPos))) {
                                holding = 2;
                            }
                            if (holding == 0) {
                                return;
                        ...
                        if (rotated) {
                            ...
                            if (holdingBest(targetPos)) {
                                holding = 1;
                            } else if (InvUtils.invSwitch(cSlot)) {
                                holding = 2;
                            }
                            if (holding == 0) {
                                return;
                            ...
                            if (crystalPos != null) {
                                mc.player.networkHandler.sendPacket(new PlayerInteractBlockC2SPacket(hand == null ? Hand.MAIN_HAND : hand,
                                    new BlockHitResult(LemonKUtils.getMiddle(crystalPos.down()), Direction.UP, crystalPos.down(), false), 0));
                            }
                    ...
                    if (rotated) {
                        ...
                        if (holdingBest(targetPos)) {
                            ...
                            switch (switchMode.get()) {
                                ...
                                case GhostHandBypass -> {
                                    if (InvUtils.invSwitch(fastestSlot(targetPos))) {
                                        holding = 2;
                                    }
                                }
                            }
                        ...
                        if (holding == 2) {
                            switch (switchMode.get()) {
                                case GhostHand -> InvUtils.swapBack();
                                case GhostHandBypass -> InvUtils.swapBack2();
                            }
                        }
    ...
    Entity isAt(BlockPos pos, BlockPos crystalPos) {
        for (Entity en : mc.world.getEntities()) {
            if ((en.getBlockPos().equals(crystalPos) && en.getType().equals(EntityType.END_CRYSTAL)) ||
                (!(en instanceof ItemEntity) &&  en.getBoundingBox().intersects(LemonKUtils.getBox(pos)))) {
                return en;
            }
        }
        return null;
    ...
    void calc() {
        ...
        if (crystalPos != null && (!LemonKUtils.isCrystalBlock(getBlock(crystalPos.down())) || !SettingUtils.inPlaceRange(crystalPos.down()) || !canPlaceCrystal(crystalPos))) {
            valid = true;
        }
    ...
    BlockPos[] getPos() {
        ...
        for (PlayerEntity pl : mc.world.getPlayers()) {
            if (pl != mc.player && !Friends.get().isFriend(pl)) {
                ...
                for (Direction dir : LemonKUtils.horizontals) {
                    // Anti Surround
                    ...
                    if (valueCheck(value, autoCity.get(), pos.offset(dir), closest) &&
                        ...
                        LemonKUtils.isCrystalBlock(getBlock(pos.offset(dir).offset(dir).down())) && SettingUtils.inMineRange(pos.offset(dir)) &&
    ...
    boolean valueCheck(int currentValue, int value, BlockPos pos, BlockPos closest) {
        ...
        if (closest == null) {
            ...
            rur = LemonKUtils.distance(LemonKUtils.getMiddle(pos), mc.player.getEyePos()) <
                LemonKUtils.distance(LemonKUtils.getMiddle(closest), mc.player.getEyePos());
        }
