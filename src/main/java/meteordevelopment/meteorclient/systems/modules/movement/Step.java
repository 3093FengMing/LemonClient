...
import meteordevelopment.meteorclient.utils.player.NewCrystalUtils;
...
public class Step extends Module {
    ...
    private double getExplosionDamage(){
        Optional<EndCrystalEntity> crystal = Streams.stream(mc.world.getEntities())
                ...
                .max(Comparator.comparingDouble(o -> NewCrystalUtils.crystalDamage(mc.player, o.getPos())))
                .map(entity -> (EndCrystalEntity) entity);
        return crystal.map(endCrystalEntity -> NewCrystalUtils.crystalDamage(mc.player, endCrystalEntity.getPos())).orElse(0.0);
    }
